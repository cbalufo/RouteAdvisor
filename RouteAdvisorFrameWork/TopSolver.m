...
-(NSArray *)run:(NSUInteger)maxAlgLoop
    ...
    //NSLog(@"step 161 count %lu",(unsigned long)[self.pois count]);
    while (algLoop < maxAlgLoop) {
        ...
        if (solutionScore > self.solutionScore) {
            ...
            if (disturbCount == 0) {
                ...
                //NSLog(@"step 167 %@",self.solutionRoutes);
                //NSLog(@"step 1671 count %lu",(unsigned long)[self.solutionRoutes count]);
    ...
    //NSLog(@"step 168 %@",_solutionRoutes);
    return self.solutionRoutes;
}
